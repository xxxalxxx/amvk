...
const aiTextureType* SkinnedModel::TEXTURE_TYPES = Model_TEXTURE_TYPES;

const uint32_t SkinnedModel::NUM_TEXTURE_TYPES = ARRAY_SIZE(Model_TEXTURE_TYPES);

SkinnedModel::SkinnedModel(VulkanState& vulkanState):
	numVertices(0),
	numIndices(0),
...
void SkinnedModel::init(std::string modelPath, unsigned int pFlags) 
{	
	init(modelPath.c_str(), pFlags);	
}

void SkinnedModel::init(const char* modelPath, unsigned int pFlags)
{
	mPath = modelPath;
...
void SkinnedModel::processModel(const aiScene& scene) 
{
...
void SkinnedModel::createCommonBuffer(const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices)
{
...
void SkinnedModel::createPipeline(VulkanState& state) 
{
...
void SkinnedModel::createDescriptorPool() 
{
...
void SkinnedModel::createDescriptorSet()
{
...
void SkinnedModel::draw(VkCommandBuffer& commandBuffer, VkPipeline& pipeline, VkPipelineLayout& pipelineLayout)
{
...
void SkinnedModel::update(VkCommandBuffer& cmdBuffer, const Timer& timer, Camera& camera)
{
	UBO ubo = {};
	ubo.model = glm::mat4();
...
void SkinnedModel::convertVector(const aiVector3D& src, glm::vec3& dest)
{
    dest.x = src.x;
    dest.y = src.y; 
    dest.z = src.z;
...
void SkinnedModel::convertVector(const aiVector3D& src, glm::vec2& dest)
{
    dest.x = src.x;
    dest.y = src.y; 
}
...
void SkinnedModel::throwError(const char* error) 
{
	std::string errorStr = error;
	throwError(errorStr);
}

void SkinnedModel::throwError(std::string& error) 
{
